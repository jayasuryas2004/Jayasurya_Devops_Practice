...
Biggest Takeaways
    ...
    Always verify running container code.
    Read error messages carefully.
    Check Content-Type when debugging Prometheus.
    Verify versions when tools fail to integrate.
    Validate every step of a log pipeline.
    HPA requires CPU requests.
    Metrics Server is mandatory for CPU-based autoscaling.
    Kubernetes troubleshooting is mostly systematic verification.
    Never assume configuration is applied correctly.
    Use kubectl describe extensively during debugging.
